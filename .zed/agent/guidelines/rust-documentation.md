...
> [!IMPORTANT]
>
> Follow these strict guidelines for doc comments (`///`) and module
> documentation (`//!`).
...
Focus on what the function does and when it fails. Use `# Errors` section only
when errors are significant.
...
Use `# Example` header for non-obvious APIs. Keep examples concise and runnable.
