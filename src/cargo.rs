...
pub fn check_nightly_installed() -> Result<()> {
    ...
    if result.status.success() {
        ...
        Err(BuildError::NightlyNotInstalled.into())
    }
}
...
pub fn generate_rustdoc_json(crate_name: &str) -> Result<std::path::PathBuf> {
    ...
    find_rustdoc_json(&doc_dir, crate_name)
}
...
fn find_rustdoc_json(doc_dir: &Path, crate_name: &str) -> Result<std::path::PathBuf> {
    // Generate candidate filenames
    let candidates = generate_filename_candidates(crate_name);
    ...
    if let Some(matching_file) = find_partial_match(doc_dir, crate_name) {
    ...
    Err(BuildError::JsonNotFound(first_candidate).into())
}
...
fn generate_filename_candidates(crate_name: &str) -> Vec<String> {
...
fn find_partial_match(doc_dir: &Path, crate_name: &str) -> Option<std::path::PathBuf> {
...
mod tests {
    ...
    //
    // These tests verify the filename matching logic that handles cargo's
    ...
    #[test]
    fn generate_filename_candidates_exact_match() {
        let candidates = generate_filename_candidates("serde");
        assert_eq!(candidates, vec!["serde.json"]);
    }

    #[test]
    fn generate_filename_candidates_with_hyphens() {
        let candidates = generate_filename_candidates("cargo-docmd");
        assert_eq!(candidates, vec!["cargo-docmd.json", "cargo_docmd.json"]);
    }

    #[test]
    fn generate_filename_candidates_with_underscores() {
        let candidates = generate_filename_candidates("cargo_docmd");
        assert_eq!(candidates, vec!["cargo_docmd.json", "cargo-docmd.json"]);
    }

    #[test]
    fn generate_filename_candidates_mixed() {
        let candidates = generate_filename_candidates("some-crate_name");
        assert_eq!(
            candidates,
            vec![
    ...
    #[test]
    fn find_partial_match_returns_none_for_nonexistent_directory() {
        let result = find_partial_match(Path::new("/nonexistent/path"), "test");
